...
class GetUsers extends Component{
    ...
    handleUpdate = (id) => {
        this.context.handleUpdate(id,this.name.value,this.name1.value,this.email.value);
    }
    ...
    render(){
        ...
        allUsers = this.context.all_users.map(({id,first_name,last_name,user_email,isEditing}) => {
            return isEditing === true ? (
                <tr key={id}>
                <td><input className="form-control" type="text" ref={(item) => this.name = item} defaultValue={first_name}/></td>
                <td><input className="form-control" type="text" ref={(item) => this.name1 = item} defaultValue={last_name}/></td>
